...
public class ClientHandler implements Server_API { //отвечает за подключенных клиентов
    ...
    public ClientHandler(Server server, Socket socket){
        ...
        new Thread(()-> { //поток с лямда выражением "()->". в отличие от ананимного класса контекст не меняется. ЦИКЛ АВТВРИЗАЦИИ
            try{
                 new Thread(()->{ //поток, который делает отключение неавторизованных клиентов через 3 секунды
                     try {
                         Thread.sleep(3000);
                     } catch (InterruptedException e) {
                         e.printStackTrace();
                     }
                     if(this.nick == "undefined" && this.socket.isConnected()){ //если клиент не авторизован а сокет открыт
                         disconnect();
                     }
                 }).start();
                //Auth
